...
# ГИБКИЙ ФИЛЬТР: Сработает на "Правила", "📜 Правила", "Правила 📜" и т.д.
@dp.message(F.text.contains("Правила"))
async def rules_menu(message: types.Message):
    rules_text = (
        "📜 **Правила магазина Ad1sKa UC**\n\n"
        "1. **Оплата:** Перевод по реквизитам. Присылайте скриншот чека в чат.\n"
        "2. **Сроки:** Зачисление происходит в рабочее время (см. График).\n"
        "3. **PUBG ID:** Проверяйте ID перед покупкой. Ошибка — ваша ответственность.\n"
        "4. **Возврат:** Невозможен после начала обработки заказа.\n\n"
...
async def save_id(message: types.Message, state: FSMContext):
    if message.text.isdigit():
        ...
        await message.answer("⚠️ Введи только цифры!")
