...
async def rules_menu(message: types.Message):
    rules = ("📜 **Правила Ad1sKa Shop:**\n\n"
             "1. UC зачисляются после 15:00 МСК.\n"
             "2. За неверный PUBG ID ответственность несет покупатель.\n"
             "3. Возврат средств после оплаты невозможен.\n"
             "4. Оплата защищена Telegram Payments.")
    await message.answer(rules, parse_mode="Markdown")

# --- КОРЗИНА И ОПЛАТА (TELEGRAM PAYMENTS) ---
...
async def checkout(callback: types.CallbackQuery):
    ...
    # Отправляем "шторку" оплаты Telegram
    await bot.send_invoice(
        ...
        currency="RUB",
        prices=[LabeledPrice(label="UC Pack", amount=total * 100)], # Сумма в копейках
        start_parameter="uc_topup"
    )

@dp.pre_checkout_query()
async def pre_checkout_query_handler(pre_checkout_q: PreCheckoutQuery):
    # Подтверждаем, что готовы принять деньги (чтобы экран "Сохранить реквизиты" исчез)
...
async def successful_payment(message: types.Message):
    ...
    user_carts[uid] = {'uc': 0, 'price': 0, 'count': 0} # Чистим корзину
    u_data = await db.get_profile(uid)
    
    await message.answer("✅ **Оплата принята!**\nUC будут зачислены после 15:00 МСК.")
    
    # Уведомляем админа о реальном платеже
    builder = InlineKeyboardBuilder()
...
async def admin_done(callback: types.CallbackQuery):
    ...
    await bot.send_message(uid, "💎 **UC зачислены!** Проверьте аккаунт.")
